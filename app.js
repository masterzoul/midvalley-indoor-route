...
function tokenMatches(p,token){
  const hay=searchable(p),t=norm(token),g=semanticFor(t);if(g)return g.match.some(m=>hay.includes(norm(m)));if(hay.includes(t))return true;
  if(t.length>=3){const words=hay.split(' ');return words.some(w=>w.startsWith(t)||(t.startsWith(w)&&w.length>=4))}return false;
}
function scorePlace(p,q,tokens){
  const nq=norm(q),name=norm(p.name),hay=searchable(p);let score=0;if(name===nq)score+=1200;else if(name.startsWith(nq))score+=900;else if(name.includes(nq))score+=700;
  for(const token of tokens){const g=semanticFor(token);if(g){if(g.match.some(m=>name.includes(norm(m))))score+=420;if(g.match.some(m=>hay.includes(norm(m))))score+=260;if((p.kind||'').toLowerCase()===g.id)score+=180}else{if(name===token)score+=300;else if(name.startsWith(token))score+=220;else if(name.includes(token))score+=160;else if(hay.includes(token))score+=90}}
  if(String(p.dataConfidence||'').startsWith('official'))score+=8;return score;
}
...
async function boot(){
  try{
    const [mv,gardens,connected,td]=await Promise.all([
      optionalJson('data/places.generated.json'),optionalJson('data/gardens.generated.json'),optionalJson('data/connected-facilities.json'),optionalJson('data/topology.generated.json')
    ]);
    ...
    const merged=new Map();for(const p of [...(mv.places||[]),...(gardens.places||[]),...(connected.places||[])])merged.set(p.id||`${p.mall}|${p.name}|${p.floor}|${p.lot}`,p);
    state.places=[...merged.values()].sort((a,b)=>a.name.localeCompare(b.name));
    const malls=new Set(state.places.map(p=>p.mall).filter(Boolean));els.dataStatus.textContent=`${state.places.length} lokasi · ${malls.size} kawasan tersambung`;
    const dates=[mv.updatedAt,gardens.updatedAt].filter(Boolean).map(x=>new Date(x)).filter(d=>!Number.isNaN(d.getTime()));const latest=dates.sort((a,b)=>b-a)[0];
...
function wirePicker(input,results,key){
  const render=()=>{const items=searchPlaces(input.value);results.innerHTML='';if(!input.value.trim()||!items.length){results.hidden=true;return}items.forEach(p=>{const b=document.createElement('button');b.type='button';b.className='result';b.innerHTML=`<strong>${escapeHtml(p.name)}</strong><span>${escapeHtml(placeMeta(p))}</span>`;b.onclick=()=>selectPlace(key,p,input,results);results.appendChild(b)});results.hidden=false};
  input.addEventListener('input',()=>{state[key]=null;render()});input.addEventListener('focus',render);input.addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();const first=searchPlaces(input.value)[0];if(first)selectPlace(key,first,input,results);input.blur()}});document.addEventListener('click',e=>{if(e.target!==input&&!results.contains(e.target))results.hidden=true});
}
