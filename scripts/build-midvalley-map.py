...
pdf = WORK / 'map.pdf'
req = urllib.request.Request(PDF_URL, headers={'User-Agent': 'Mozilla/5.0 MidValleyIndoorRoute/3.0'})
...
# Correct floor bands determined from the official stacked map artwork.
bands=[
    ('3',0.010,0.195),
    ('2',0.205,0.300),
    ('1',0.310,0.405),
...
def canon(text):
    ...
    x=re.sub(r'[^A-Z0-9()/-]','',x)
    # Common OCR confusions inside lot codes.
...
def official_codes(lot):
    x=canon(lot)
    return set(lot_token_re.findall(x))
...
# Group OCR words by Tesseract line so ranges such as G038 - G039 are kept together.
...
for words in groups.values():
    ...
    line=''.join(wd[4] for wd in words)
    c=canon(line)
    tokens=lot_token_re.findall(c)
    ...
    for token in tokens:
        ...
        if not expected and token not in code_to_tenants:continue
        key=(floor,token,round(cx/10),round(cy/10))
        if key in seen:continue
        seen.add(key);matched_codes.add(token)
        candidates.append({
            'code':token,'ocrLine':line,'floor':floor,'x':round(cx,1),'y':round(cy,1),
            'xNorm':round(cx/w,7),'yNorm':round(cy/h,7),
            'confidence':round(sum(z[5] for z in words)/len(words),2),
            'tenants':expected
        })

# Also retain unmatched lot-looking OCR lines for audit, without using them as route truth.
audit=[]
for words in groups.values():
    ...
    audit.append({'text':line,'canonical':c,'tokens':tokens,'floor':floor_for(cy),'xNorm':round(cx/w,7),'yNorm':round(cy/h,7)})

payload={
    'schemaVersion':2,'source':PDF_URL,'generatedAt':datetime.now(timezone.utc).isoformat(),
    'image':{'width':w,'height':h,'dpi':600},'floorBands':[{'floor':f,'from':lo,'to':hi} for f,lo,hi in bands],
    'palette':palette,'matchedLotLabels':candidates,'matchedCodeCount':len(matched_codes),'ocrAudit':audit,
    'note':'Coordinates are accepted only when an OCR lot token matches a current official Mid Valley tenant lot on the same floor. Corridor/left-right routing requires separate walkable-path validation.'
}
OUT.parent.mkdir(parents=True,exist_ok=True);OUT.write_text(json.dumps(payload,indent=2)+'\n',encoding='utf-8')
print(f'Official floor map {w}x{h} @600dpi; matched {len(candidates)} lot-label positions across {len(matched_codes)} official lot codes; {len(audit)} lot-like OCR lines audited.')
